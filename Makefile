...
$(x86_64_kernel_object): build/kernel/%.o : src/impl/kernel/%.c
	mkdir -p $(dir $@) && \
	gcc -c -I src/intf -ffreestanding $(patsubst build/kernel/%.o, src/impl/kernel/%.c, $@) -o $@ -ggdb -fstack-protector


$(x86_64_c_object): build/x86_64/%.o : src/impl/x86_64/%.c
	mkdir -p $(dir $@) && \
	gcc -c -I src/intf -ffreestanding $(patsubst build/x86_64/%.o, src/impl/x86_64/%.c, $@) -o $@ -fstack-protector
...
build-x86_64: $(x86_64_kernel_object) $(x86_64_object)
	mkdir -p dist/x86_64 && cd .
	ld -n -o dist/x86_64/kernel.bin -T targets/x86_64/linker.ld $(x86_64_kernel_object) $(x86_64_object)
